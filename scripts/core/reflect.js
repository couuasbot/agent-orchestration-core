...
function recall(query, context = {}) {
  ...
  if (results.length) {
    ...
    appendEvent({ type: 'AOS_RECALL', agent: 'god', payload });
  } else {
    console.log('No specific patterns found. Proceed with standard protocol.');
    appendEvent({ type: 'AOS_RECALL', agent: 'god', payload: { ...payload, matches: [] } });
  }
}

function learn(lesson, context = {}) {
  ensureFiles();
  ...
  appendEvent({ type: 'AOS_LEARN', agent: 'god', payload });
...
for (const arg of args) {
  ...
  if (arg.startsWith('--taskId=')) taskId = arg.split('=').slice(1).join('=');
  if (arg.startsWith('--runId=')) runId = arg.split('=').slice(1).join('=');
}
...
if (action === 'recall') {
  ...
  learn(lesson, context);
} else {
  console.log('Usage: node reflect.js --action=recall --query="deploy" [--taskId=...] [--runId=...]');
}
